...
function setCertificateData(result) {
	...
	let idDate = result.Certificate;
	layui.use('form', function () {
		if (idDate != undefined) {
			$('.IDname').val(idDate.Name);
			$('#IDcard').val(idDate.IDNumber);
			$('.address').val(idDate.Address);
			$("#idcardimg").attr("src", "data:image/jpg;base64," + idDate.Base64Photo);
			...
			let date = new Date(),
				birth = idDate.Birthday,
				y1 = birth.toString().substring(-1, 4),
				year = date.getFullYear() - y1,
				m1 = birth.substring(4, 6),
				m2 = date.getMonth() + 1;
			if (m1 < m2) {
				// console.log(year)
				$('#age').attr("value", year);
